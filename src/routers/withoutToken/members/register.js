...
module.exports = router => {
    router.post("/", async (req, res) => {
        try {
            let member = req.body
            let errors = []
            let checkEmail = await mongoose.model("member").find({ email: member.email }).then(e => {
                console.log(e.length)
                if (e.length !== 0)
                    errors.push('Email')
            })
            let checkUsername = await mongoose.model("member").find({ username: member.username }).then(e => {
                if (e.length !== 0)
                    errors.push('Tên đăng nhập')
            })
            let checkPhone = await mongoose.model("member").find({ phone: member.phone }).then(e => {
                if (e.length !== 0)
                    errors.push('Điện thoại')
            })
            if (errors.length > 0) {
                return res.status(400).send(errors)
            }
            await mongoose.model("member").create(member).then(data => member = data)
